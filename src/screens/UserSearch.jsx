...
const UserSearch = () => {
  ...
  const sort = sortType => {
    ...
    if (sortType === SortOrder.DESCENDING) {
      const sortedResult = searchContext.searchResult.sort((a, b) =>
        b.email.localeCompare(a.email)
      )
      newContext.searchResult = [...sortedResult]
      setSearchContext(newContext)
    } else if (sortType === SortOrder.ORIGINAL) {
      newContext.searchResult = [...searchContext.originalOrder]
      setSearchContext(newContext)
    } else {
      const sortedResult = searchContext.searchResult.sort((a, b) =>
  ...
  return (
    <div>
      ...
      <div>
        <button onClick={() => sort(SortOrder.ASCENDING)}>
          Sort Ascending by email
        </button>
        <button onClick={() => sort(SortOrder.DESCENDING)}>
          Sort descending by email
        </button>
        <button onClick={() => sort(SortOrder.ORIGINAL)}>
          Sort orignal by email
        </button>
      </div>
        ...
        <table>
          ...
          <tbody>
            {searchContext.searchResult &&
              searchContext.searchResult.map(item => (
